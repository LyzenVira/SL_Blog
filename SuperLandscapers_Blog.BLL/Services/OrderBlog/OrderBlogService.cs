...
namespace BLL.Services.OrderBlog
{
    public class OrderBlogService : IOrderBlogService
    {
        ...
        public async Task<ResponseEntity<IEnumerable<GetOrderBlogDTO>>> GetPagedOrderBlogsAsync(int page, int pageSize)
        {
            var orderBlogs = _wrapperRepository.OrderBlogRepository.GetPagedOrders(page, pageSize);
            return new ResponseEntity<IEnumerable<GetOrderBlogDTO>>(System.Net.HttpStatusCode.OK, _mapper.Map<IEnumerable<GetOrderBlogDTO>>(orderBlogs));
        }

        public async Task<ResponseEntity<IEnumerable<GetOrderBlogDTO>>> GetFilteredOrderBlogsAsync(string username)
        {
            var orderBlogs = _wrapperRepository.OrderBlogRepository.GetFilteredOrders(username);
            return new ResponseEntity<IEnumerable<GetOrderBlogDTO>>(System.Net.HttpStatusCode.OK, _mapper.Map<IEnumerable<GetOrderBlogDTO>>(orderBlogs));
        }

        public async Task<ResponseEntity<IEnumerable<GetOrderBlogDTO>>> GetSortedOrderBlogsAsync(string sortBy)
        {
            var orderBlogs = _wrapperRepository.OrderBlogRepository.GetSortedOrders(sortBy);
            return new ResponseEntity<IEnumerable<GetOrderBlogDTO>>(System.Net.HttpStatusCode.OK, _mapper.Map<IEnumerable<GetOrderBlogDTO>>(orderBlogs));
        }
    }
}
